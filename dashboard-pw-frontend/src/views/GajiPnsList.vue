    ...
    <v-main>
      <v-container fluid class="pa-8">
        ...
        <div class="d-flex align-center mb-6">
          <div>
            <h1 class="text-h5 font-weight-bold">Data Gaji PNS</h1>
            <p class="text-body-2 text-medium-emphasis mt-1">Kelola data gaji Pegawai Negeri Sipil</p>
          </div>
          <v-spacer></v-spacer>
              ...
              <v-card class="glass-card rounded-xl mb-4 pa-4" elevation="0">
                ...
                <div class="stat-item mb-3">
                  <div class="d-flex align-center justify-space-between">
                    ...
                    <v-chip size="small" color="primary" variant="flat">{{ stats.total.toLocaleString('id-ID') }}</v-chip>
                  </div>
                </div>
                <div class="stat-item mb-3">
                  <div class="d-flex align-center justify-space-between">
                    <span class="text-body-2 text-grey">Gaji Pokok</span>
                    <span class="text-caption font-weight-bold text-primary">{{ formatCurrency(stats.total_gaji_pokok) }}</span>
                  </div>
                </div>
                <div class="stat-item mb-3">
                  <div class="d-flex align-center justify-space-between">
          ...
          <v-col cols="12" md="9">
            <v-card class="glass-card rounded-xl overflow-hidden" elevation="0">
              <v-toolbar color="transparent" flat class="px-6 py-4">
                <v-toolbar-title class="font-weight-bold text-h6">Daftar Gaji PNS</v-toolbar-title>
                <v-spacer></v-spacer>
                <v-text-field
                  v-model="search"
              ...
              >
                ...
                <template v-slot:item.nama="{ item }">
                  <div class="d-flex align-center py-2">
                    <v-avatar size="36" color="indigo-lighten-5" class="mr-3">
                      <v-icon color="indigo" size="small">mdi-account-tie</v-icon>
                    </v-avatar>
                    <div>
                ...
                <template v-slot:item.jenis_gaji="{ item }">
                  <v-chip size="x-small" :color="item.jenis_gaji === 'Induk' ? 'primary' : 'orange'" variant="tonal" class="font-weight-bold">
                    {{ item.jenis_gaji || 'Induk' }}
                  </v-chip>
                </template>
                ...
                <template v-slot:item.bersih="{ item }">
                  <span class="font-weight-bold text-primary">{{ formatCurrency(item.bersih) }}</span>
                </template>
                ...
                <template v-slot:no-data>
                  <div class="text-center py-12">
                    ...
                    <div class="text-grey mt-2">Tidak ada data gaji PNS.</div>
                  </div>
                </template>
      ...
      <v-card class="rounded-xl">
        ...
        <v-card-text class="pa-6">
          <v-form ref="formRef" v-model="formValid">
            <v-row dense>
              <!-- Periode -->
              <v-col cols="12"><div class="text-overline text-primary mb-2">Periode</div></v-col>
              <v-col cols="6" md="3">
              ...
              <!-- Data Pegawai -->
              <v-col cols="12" class="mt-4"><div class="text-overline text-primary mb-2">Data Pegawai</div></v-col>
              <v-col cols="12" md="4">
              ...
              <!-- Potongan -->
              <v-col cols="12" class="mt-4"><div class="text-overline text-primary mb-2">Potongan</div></v-col>
              <v-col cols="12" md="3">
        ...
        <v-card-actions class="pa-6 pt-0">
          ...
          <v-btn color="primary" variant="flat" @click="saveRecord" :loading="saving" :disabled="!formValid">
    ...
    <v-dialog v-model="detailDialog" max-width="800px" scrollable>
      <v-card class="rounded-xl overflow-hidden glass-card">
        <v-toolbar color="indigo" dark>
          <v-toolbar-title>Detail Gaji PNS</v-toolbar-title>
          <v-spacer></v-spacer>
        ...
        <v-card-text class="pa-6" v-if="selectedRecord">
          ...
          <div class="mb-4">
            ...
            <div class="text-body-2 text-primary font-weight-bold">NIP: {{ selectedRecord.nip }}</div>
            <v-chip size="small" color="primary" variant="tonal" class="mt-1 mr-1">{{ selectedRecord.golongan || '-' }}</v-chip>
          ...
          <!-- Tunjangan -->
          <div class="text-overline text-primary mt-4 mb-2">Gaji & Tunjangan</div>
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await api.get('/gaji-pns', { params })
...
const saveRecord = async () => {
  ...
  try {
    ...
    let response
    if (isEditing.value) {
      response = await api.put(`/gaji-pns/${form.value.id}`, payload)
    } else {
      response = await api.post('/gaji-pns', payload)
    }
    if (response.data.success) {
...
const deleteRecord = async () => {
  deleting.value = true
  try {
    const response = await api.delete(`/gaji-pns/${recordToDelete.value.id}`)
    if (response.data.success) {
...
const exportExcel = async () => {
  ...
  try {
    ...
    const response = await api.get('/gaji-pns/export', {
      params,
      responseType: 'blob',
    })
    ...
    let filename = 'Gaji_PNS_Export.xlsx'
