...
class GajiPppkController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('search') && $request->search) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('gaji_pppk.nama', 'like', "%{$search}%")
                    ...
                    ->orWhere('gaji_pppk.skpd', 'like', "%{$search}%")
        ...
        // Available periods
        $periods = GajiPppk::select('bulan', 'tahun')
            ->distinct()
            ->orderBy('tahun', 'desc')
            ->orderBy('bulan', 'desc')
            ->get();
        ...
        $skpdQuery = GajiPppk::select('gaji_pppk.kdskpd', DB::raw('COALESCE(sm.nama_skpd, s1.nmsatker, s2.nmskpd, gaji_pppk.skpd) as skpd'))
        ...
        if ($request->has('bulan') && $request->has('tahun')) {
            $skpdQuery->where('gaji_pppk.bulan', $request->bulan)->where('gaji_pppk.tahun', $request->tahun);
        }
        $skpds = $skpdQuery->get();

        $data = $query->orderBy('gaji_pppk.skpd')->orderBy('gaji_pppk.nama')
    ...
    public function store(Request $request)
    {
        ...
        // Check if posted
        if (PayrollPosting::isLocked($validated['tahun'], $validated['bulan'], 'PPPK')) {
            return response()->json([
                'success' => false,
        ...
        return response()->json([
            'success' => true,
            'message' => 'Data gaji PPPK berhasil ditambahkan',
            'data' => $record,
        ], 201);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (PayrollPosting::isLocked($record->tahun, $record->bulan, 'PPPK')) {
            return response()->json([
                'success' => false,
        ...
        return response()->json([
            'success' => true,
            'message' => 'Data gaji PPPK berhasil diperbarui',
            'data' => $record,
        ]);
    }
