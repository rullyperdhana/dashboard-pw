...
class GajiPnsExport implements FromQuery, WithHeadings, WithStyles, ShouldAutoSize, WithMapping, WithColumnFormatting
{
    protected $filters;
    ...
    public function query()
    {
        ...
        if (!empty($this->filters['bulan']) && !empty($this->filters['tahun'])) {
            $query->where('gaji_pns.bulan', $this->filters['bulan'])
                ->where('gaji_pns.tahun', $this->filters['tahun']);
        } elseif (!empty($this->filters['tahun'])) {
            $query->where('gaji_pns.tahun', $this->filters['tahun']);
        }

        // Filter by SKPD
        if (!empty($this->filters['kdskpd'])) {
            $query->where('gaji_pns.kdskpd', $this->filters['kdskpd']);
        }

        // Filter by jenis gaji
        if (!empty($this->filters['jenis_gaji'])) {
            $query->where('gaji_pns.jenis_gaji', $this->filters['jenis_gaji']);
        }
        ...
        return $query->orderBy('gaji_pns.skpd')->orderBy('gaji_pns.nama');
    }
